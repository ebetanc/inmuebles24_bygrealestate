-- Anchor the EasyBroker correlation window to the first creation-worker claim,
-- not to the original I24 capture. This prevents a late final assignment from
-- producing a POST that can no longer be found by the posterior GET.
...
COMMENT ON FUNCTION public.claim_v3_easybroker_request_creations(INTEGER,TIMESTAMPTZ,INTERVAL)
  IS 'Claims eligible EasyBroker request creations and anchors the first never-posted claim to a bounded 24-hour posterior-GET correlation window.';
