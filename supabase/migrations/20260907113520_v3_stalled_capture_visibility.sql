-- Expose stalled captures and missing/partial EasyBroker closure. No routing changes.
-- Keep the existing columns, invoker security and grants.
-- Rollback: restore the backed-up live view definition.
...
eb AS (
  ...
  SELECT
    ...
    bool_and(lg.attended_state = 'succeeded') AS attended_ok,
