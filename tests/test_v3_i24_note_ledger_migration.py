"""Runs tests/sql/test_v3_i24_note_ledger.sql against an ephemeral PG17.

The whole schema chain (whatsapp-agent/migrations + supabase/migrations, in
order) is applied first, so the fixture exercises the live trigger and function
bodies and not a hand-rolled subset. Skipped when no docker daemon is reachable;
the static contract lives in tests/test_v3_i24_note_ledger_migration_draft.py.
"""
...
FIXTURE = ROOT / "tests" / "sql" / "test_v3_i24_note_ledger.sql"
...
INSERT INTO agents (agent_id,name,whatsapp_number,on_shift,is_available) VALUES
 ...
 ('agent_gina','Georgina Perez','+5215500000104',true,true),
...
def test_migration_and_rollback_fixture_pass_on_pg17():
    name = f"v3-i24-note-gate-{uuid.uuid4().hex[:8]}"
    ...
    try:
        ...
        assert "V3_I24_NOTE_LEDGER_TESTS_PASS" in run.stderr + run.stdout
    finally:
        _docker("rm", "-f", name)
