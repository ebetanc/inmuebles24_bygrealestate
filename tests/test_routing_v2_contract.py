...
def test_lrv2_008_offer_delivery_workflows_are_correlated_and_idempotent():
    ...
    requires(raw13, r"tomar prospecto", r"claim:", r"create_delivery_attempt", r"bind_delivery_message", r"should send\?", r"should_send")
    wf13_queries = "\n".join(
    ...
    assert "n8n-nodes-base.webhook" not in raw1
    requires(raw22, r"binary.*data.*data", r"buffer.from.*base64", r"timingsafeequal", r"wf1_workflow_id")
    requires(normalized(json.dumps(wf23)), r"sweep_owner_delivery_no_callback", r"interval '2 minutes'")
    ...
    assert wf13["connections"]["Create Delivery Attempt"]["main"][0][0]["node"] == "Should Send?"
    ...
    for source, export_name in (
        ...
        for edition in (export, export.get("activeVersion")):
            assert edition is not None, f"{export_name} needs a versioned active snapshot"
...
def test_s04_night_window_and_0805_drain_are_central_and_idempotent():
    ...
    assert wf7["connections"]["Process One by One"]["main"][1][0]["node"] == "Call WF3a: Auction Launcher"
    assert wf7["connections"]["Call WF3a: Auction Launcher"]["main"][0][0]["node"] == "Acknowledge Durable Handoff"
    ...
    requires(night_sql, r"upsert_lead_opportunity\s*\(", r"'queued_night'")
    ...
    for edition in (wf7_export, wf7_export["activeVersion"]):
    ...
    for edition in (wf10_export, wf10_export["activeVersion"]):
        export_sql = normalized(json.dumps(edition))
        requires(export_sql, r"upsert_lead_opportunity", r"where \$7::bigint is not null")
...
def test_s05_s06_s07_tiers_are_sequential_and_end_unassigned():
    ...
    requires(raw, r"sweep_expired_routing_tiers", r"claim_pending_guard_deliveries", r"wf13_workflow_id")
    requires(raw, r"lease_token", r"target_agent_id")
    assert "agent_manager" not in raw
    assert "assign manager" not in raw
    assert "fetch agent pool" not in raw
    assert wf3c["connections"]["Route Transition"]["main"][0][0]["node"] == "Claim Pending Guard Deliveries"
    assert wf3c["connections"]["Route Transition"]["main"][1][0]["node"] == "Alert Unassigned (No Assignment)"
    ...
    requires(wf13_raw, r"a\.lease_token=\$7", r"target_agent_id.*\$json\.target_agent_id", r"bind_delivery_message")
...
def test_s08_s09_claim_rpc_is_atomic_and_late_claim_cannot_reassign():
    ...
    assert "/^claim:(\\d+):(owner|primary_guard|backup_guard)$/" in wf1_code
    requires(wf3b_raw, r"claim_lead_opportunity", r"claim_status", r"is routing v2 claim\?")
    requires(wf3b_raw, r"not exists.*lead_routing_opportunities", r"digest\(\$4,'sha256'\)")
    ...
    assert wf3b["connections"]["Is Routing V2 Claim?"]["main"][1][0]["node"] == "Atomic Claim (WHERE status=open)"
    ...
    for edition in (export, export.get("activeVersion")):
        ...
        assert {node["name"]: node for node in edition["nodes"]} == source_nodes
        assert edition["connections"] == wf3b["connections"]
...
def test_s11_first_tag_is_strict_and_missing_owner_falls_back_to_guards():
    ...
    assert webhook["credentials"]["httpHeaderAuth"]["id"] == "REPLACE_WITH_SCRAPER_HEADER_AUTH_CREDENTIAL_ID"
    assert wf10["connections"]["Scraper Webhook"]["main"][0][0]["node"] == "Split & Normalize Leads"
    if wf10.get("activeVersion"):
        active_webhook = next(n for n in wf10["activeVersion"]["nodes"] if n["name"] == "Scraper Webhook")
        assert active_webhook["parameters"]["authentication"] == "headerAuth"
        assert wf10["activeVersion"]["connections"] == wf10["connections"]
...
def test_canonical_opportunity_upsert_is_concurrent_safe_and_auditable():
    ...
    requires(wf10_sql, r"upsert_lead_opportunity\s*\([^;]+\$11[^;]+captured", r"upsert_lead_opportunity\s*\([^;]+\$7[^;]+queued_night")
    ...
    requires(wf10_sql, r"upsert_lead_opportunity\s*\([^,]+,\s*\$2,\s*null,")
    assert wf10_sql.count("upsert_lead_opportunity") >= 3
    ...
    assert wf10["connections"]["Prepare Routing Data"]["main"][0][0]["node"] == "Upsert Canonical Opportunity"
    for workflow in (wf2, wf8, wf10):
        for connection in workflow["connections"].values():
            assert isinstance(connection["main"], list)
            assert all(isinstance(branch, list) for branch in connection["main"])
...
def test_lrv2_013_watchdog_reports_routing_failures_idempotently():
    ...
    trigger_names = [n["name"] for n in wf20["nodes"] if n["type"] == "n8n-nodes-base.scheduleTrigger"]
    assert len(trigger_names) >= 2, "watchdog needs a dedicated safe-mode cadence, not just the 30-min scraper cron"
    alert_node = next(
    ...
    safe_mode_triggers = [
        ...
        if wf20["connections"][t]["main"][0][0]["node"] == report_node["name"]
    ]
    assert safe_mode_triggers, "no schedule trigger feeds the routing-failure report node"
...
def test_lrv2_013_safe_mode_intake_skips_owner_and_routes_direct_to_guard():
    wf10 = json.loads((WORKFLOWS / "WF10_scraper_intake.json").read_text(encoding="utf-8"))
    raw = normalized(json.dumps(wf10))
    requires(raw, r"get_routing_safe_mode", r"route_missing_owner_data")

    node_params = {n["name"]: n["parameters"] for n in wf10["nodes"]}
    assert "Check Routing Safe Mode" in node_params
    assert "get_routing_safe_mode" in json.dumps(node_params["Check Routing Safe Mode"])
    assert "Safe Mode Active?" in node_params

    direct_node_name = next(
        n["name"] for n in wf10["nodes"]
        if n["name"] != "Route Missing Owner Data"
        and "route_missing_owner_data" in n.get("parameters", {}).get("query", "")
    )
    direct_node = node_params[direct_node_name]
    assert "'routing_safe_mode'" in direct_node["query"]
    assert "upsert canonical opportunity" in normalized(json.dumps(direct_node))

    # LRV2-013 P1-A: the pre-existing owner-fallback node keeps its own distinct reason.
    fallback_node = node_params["Route Missing Owner Data"]
    assert "'missing_owner_data'" in fallback_node["query"]

    migration_0028 = (MIGRATIONS / "0028_routing_safe_mode.sql").read_text(encoding="utf-8")
    assert "CREATE OR REPLACE FUNCTION public.route_missing_owner_data" in migration_0028
    assert "'missing_owner_data', 'routing_safe_mode'" in migration_0028

    conn = wf10["connections"]
    # Should Route? still gates intake the same way, but now feeds the safe-mode check first.
    assert conn["Should Route?"]["main"][0][0]["node"] == "Check Routing Safe Mode"
    assert conn["Should Route?"]["main"][1][0]["node"] == "Stop Duplicate Intake"
    assert conn["Check Routing Safe Mode"]["main"][0][0]["node"] == "Safe Mode Active?"
    safe_branch = conn["Safe Mode Active?"]["main"]
    assert safe_branch[0][0]["node"] == direct_node_name
    assert safe_branch[1][0]["node"] == "Resolve Owner (WF12)"
    # The existing owner-first path (WF12 -> WF13) is untouched.
    assert conn["Resolve Owner (WF12)"]["main"][0][0]["node"] == "Owner Resolved?"
    assert conn["Owner Resolved?"]["main"][0][0]["node"] == "Prepare Owner Notify"
    assert conn["Owner Resolved?"]["main"][1][0]["node"] == "Route Missing Owner Data"
    # Both the pre-existing fallback and the new direct-to-guard path terminate the same way.
    assert conn["Route Missing Owner Data"]["main"][0][0]["node"] == "End (Owner Fallback)"
    assert conn[direct_node_name]["main"][0][0]["node"] == "End (Owner Fallback)"
...
def test_lrv2_014_wf20_alerts_unassigned_cases_via_dedupe_rpc():
    ...
    # The existing safe-mode branch stays intact...
    assert conn["Cada 5 min (modo seguro)"]["main"][0][0]["node"] == "Revisar Fallos de Routing"
    ...
    # ...and the same trigger now also fans out to the unassigned-case check.
    assert conn["Cada 5 min (modo seguro)"]["main"][0][1]["node"] == "Revisar Casos Sin Asignar"
