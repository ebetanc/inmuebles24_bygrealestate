...
Gate E2E real (2026-08-13, ventana autorizada): PASS — 1 WhatsApp autorizado exacto, callback Meta firmado, late claim rechazado, claim aceptado/idempotente, restauración hash-exacta; 3 defectos (credencial placeholder, executeWorkflow 1.2 string plano, pgcrypto search_path) portados al repo en `fix/lrv2-e2e-fixes` con migración `0034`, mapping fail-closed de credenciales y regresión offline `tests/test_lrv2_e2e_regression.py`; gate PG17 rehecho con pgcrypto en schema `extensions` (falso verde eliminado), fixtures 7/7 PASS.
