...
```
Inmuebles24 (solo bandeja mensajes) --15 min--> scraper (Raspberry Pi)
      |                                  |
...
1. **Captura.** El scraper (`src/inmobiliaria24/`) recorre la bandeja
   `mensajes` de Inmuebles24 cada 15 minutos, 24/7, y persiste cada solicitud en
   `v3_intake` antes de tocar nada más. Reintentar el mismo evento no crea otra
   oportunidad. Las bandejas `telefono` ("Vió teléfono") y `whatsapp` ("Contactó
   por Whatsapp") se ignoran: registran que alguien reveló el número o tocó el
   botón, no que haya escrito. Subastarlas producía llamadas en frío a quien no
   había pedido nada.
